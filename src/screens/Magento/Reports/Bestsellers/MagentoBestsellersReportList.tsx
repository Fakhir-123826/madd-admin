...
function MagentoBestsellersReportList() {
  ...
  const bestsellers = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === bestsellers.length ? [] : bestsellers.map((i: any) => i.id)
    );
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 border border-gray-200 rounded-2xl px-5 py-3 bg-gray-50 focus-within:border-teal-400 focus-within:bg-white transition-all w-full md:w-96">
            ...
            <input
              ...
              placeholder="Search by Product or Interval..."
          ...
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === bestsellers.length && bestsellers.length > 0}
                  onChange={toggleAll}
              ...
              <th className={thClass}>Order Quantity</th>
            </tr>
          </thead>
          <tbody>
            {bestsellers.length === 0 ? (
              ...
              bestsellers.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className={tdClass}>{item.order_quantity}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoBestsellersReportList;