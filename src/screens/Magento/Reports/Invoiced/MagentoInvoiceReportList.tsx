...
function MagentoInvoiceReportList() {
  ...
  const invoices = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === invoices.length ? [] : invoices.map((i: any) => i.id)
    );
  };
        ...
        <table className="w-full min-w-[1100px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              ...
              <th className={thClass}>Invoiced Orders</th>
              <th className={thClass}>Total Invoiced</th>
              <th className={thClass}>Paid Invoices</th>
              <th className={thClass}>Unpaid Invoices</th>
            </tr>
          </thead>
          <tbody>
            {invoices.length === 0 ? (
              ...
              invoices.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className={tdClass}>{item.invoiced_orders}</td>
                  <td className={tdClass}>{item.total_invoiced}</td>
                  <td className={tdClass}>{item.paid_invoices}</td>
                  <td className={tdClass}>{item.unpaid_invoices}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoInvoiceReportList;