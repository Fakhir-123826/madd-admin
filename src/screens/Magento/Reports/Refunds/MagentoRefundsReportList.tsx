...
function MagentoRefundsReportList() {
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === refunds.length ? [] : refunds.map((i: any) => i.id)
    );
  };
          ...
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === refunds.length && refunds.length > 0}
                  onChange={toggleAll}
                  className="w-4 h-4 accent-white"
                />
              </th>
              <th className={thClass}>Interval</th>
              <th className={thClass}>Refunded Orders</th>
              <th className={thClass}>Total Refunded</th>
              <th className={thClass}>Online Refunds</th>
              <th className={thClass}>Offline Refunds</th>
            </tr>
          </thead>
          <tbody>
            {refunds.length === 0 ? (
              ...
              refunds.map((item: any, idx: number) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className={tdClass}>{item.refunded_orders}</td>
                  <td className={tdClass}>{item.total_refunded}</td>
                  <td className={tdClass}>{item.online_refunds}</td>
                  <td className={tdClass}>{item.offline_refunds}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoRefundsReportList;