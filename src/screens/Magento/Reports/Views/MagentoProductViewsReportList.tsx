...
function MagentoProductViewsReportList() {
  ...
  const productViews = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === productViews.length ? [] : productViews.map((i: any) => i.id)
    );
  };
        ...
        <table className="w-full min-w-[900px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === productViews.length && productViews.length > 0}
                  onChange={toggleAll}
          ...
          <tbody>
            {productViews.length === 0 ? (
              ...
              productViews.map((item: any, idx: number) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className={tdClass}>{item.views}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoProductViewsReportList;