...
function MagentoTaxReportList() {
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === taxData.length ? [] : taxData.map((i: any) => i.id)
    );
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 border border-gray-200 rounded-2xl px-5 py-3 bg-gray-50 focus-within:border-teal-400 focus-within:bg-white transition-all w-full md:w-96">
            ...
            <input
              ...
              placeholder="Search by Interval, Tax, Rate..."
      ...
      {showFilters && (
        <div className="bg-gray-100 border-b px-6 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Date Used */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Date Used</div>
              ...
              <p className="text-[10px] text-gray-400 mt-1">The Order Updated report is created in real time and does not require a refresh.</p>
            </div>
            ...
            {/* Order Status */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Order Status</div>
              <select className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400">
                <option>Any</option>
              </select>
            </div>
            ...
            {/* Empty Rows */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Empty Rows</div>
              <select className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400">
                <option>No</option>
              </select>
            </div>
            ...
            {/* Show Actual Values */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Show Actual Values</div>
              <select className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400">
                <option>No</option>
              </select>
            </div>
          ...
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === taxData.length && taxData.length > 0}
                  onChange={toggleAll}
              ...
              <th className={thClass}>Tax Amount</th>
            </tr>
          </thead>
          <tbody>
            {taxData.length === 0 ? (
              ...
              taxData.map((item: any, idx: number) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className={tdClass}>{item.tax}</td>
                  <td className={tdClass}>{item.rate}</td>
                  <td className={tdClass}>{item.orders}</td>
                  <td className={tdClass}>{item.tax_amount}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoTaxReportList;
