...
function MagentoCouponsReportList() {
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === coupons.length ? [] : coupons.map((i: any) => i.id)
    );
  };
      ...
      {showFilters && (
        <div className="bg-gray-100 border-b px-6 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Cart Price Rule */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Cart Price Rule</div>
              <select className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400">
                <option>Any</option>
              </select>
            </div>
        ...
        <table className="w-full min-w-[1100px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              ...
              <th className={thClass}>Coupon Code</th>
              <th className={thClass}>Price Rule</th>
              <th className={thClass}>Uses</th>
              <th className={thClass}>Sales Subtotal</th>
              <th className={thClass}>Sales Discount</th>
            ...
            {coupons.length === 0 ? (
              ...
              coupons.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className={tdClass}>{item.coupon_code}</td>
                  <td className={tdClass}>{item.price_rule}</td>
                  <td className={tdClass}>{item.uses}</td>
                  <td className={tdClass}>{item.sales_subtotal}</td>
                  <td className={tdClass}>{item.sales_discount}</td>
                  <td className={tdClass}>{item.sales_total}</td>
...
export default MagentoCouponsReportList;