...
function MagentoBraintreeSettlementReportList() {
  ...
  const settlements = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === settlements.length ? [] : settlements.map((i: any) => i.id)
    );
  };
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
      ...
      {/* HEADER with Search Bar + Filters Toggle */}
      ...
      {showFilters && (
        <div className="bg-gray-100 border-b px-6 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Created At */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Created At</div>
            ...
            {/* Amount */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Amount</div>
              <div className="grid grid-cols-2 gap-3">
                <input type="text" placeholder="From" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
                <input type="text" placeholder="To" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
              </div>
            </div>
            ...
            <div>
              ...
              <input type="text" placeholder="Transaction ID" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
            </div>

            {/* Order ID */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Order ID</div>
              <input type="text" placeholder="Order ID" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
            </div>

            {/* PayPal Payment ID */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">PayPal Payment ID</div>
              <input type="text" placeholder="PayPal Payment ID" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
            </div>

            {/* Payment Type */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Payment Type</div>
            ...
            {/* Settlement Code */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Settlement Code</div>
              <input type="text" placeholder="Settlement Code" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
            </div>
            ...
            {/* Merchant Account ID */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Merchant Account ID</div>
              <input type="text" placeholder="Merchant Account ID" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
            </div>

            {/* Settlement Batch ID */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Settlement Batch ID</div>
              <input type="text" placeholder="Settlement Batch ID" className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" />
            </div>
          </div>
        </div>
      )}
        ...
        <table className="w-full min-w-[1300px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              ...
              <th className={thClass}>PayPal Payment ID</th>
              <th className={thClass}>Payment Type</th>
              <th className={thClass}>Transaction Type</th>
              <th className={thClass}>Created At</th>
              <th className={thClass}>Amount</th>
              <th className={thClass}>Settlement Code</th>
              <th className={thClass}>Status</th>
              <th className={thClass}>Settlement Response Text</th>
              <th className={thClass}>Refund Ids</th>
              <th className={thClass}>Merchant Account ID</th>
              <th className={thClass}>Settlement Batch ID</th>
            ...
            {settlements.length === 0 ? (
              ...
              settlements.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className={tdClass}>{item.transaction_id}</td>
                  <td className={tdClass}>{item.order_id}</td>
                  <td className={tdClass}>{item.paypal_payment_id}</td>
                  <td className={tdClass}>{item.payment_type}</td>
                  <td className={tdClass}>{item.transaction_type}</td>
                  <td className={tdClass}>{item.created_at}</td>
                  <td className={tdClass}>{item.amount}</td>
                  <td className={tdClass}>{item.settlement_code}</td>
                  <td className={tdClass}>{item.status}</td>
                  <td className={tdClass}>{item.settlement_response_text}</td>
                  <td className={tdClass}>{item.refund_ids}</td>
                  <td className={tdClass}>{item.merchant_account_id}</td>
                  <td className={tdClass}>{item.settlement_batch_id}</td>
                  <td className={tdClass}>{item.currency}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoBraintreeSettlementReportList;