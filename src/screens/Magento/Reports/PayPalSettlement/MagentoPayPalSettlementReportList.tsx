...
function MagentoPayPalSettlementReportList() {
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 border border-gray-200 rounded-2xl px-5 py-3 bg-gray-50 focus-within:border-teal-400 focus-within:bg-white transition-all w-full md:w-96">
            ...
            <input
              ...
              placeholder="Search by Transaction ID, Invoice ID..."
        ...
        <div className="bg-gray-100 border-b px-6 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-6">
            ...
            <div className="lg:col-span-2">
              <div className="text-xs text-gray-500 mb-1 font-medium">Report Date</div>
            ...
            {/* Merchant Account */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Merchant Account</div>
              <input 
                type="text" 
                placeholder="Merchant Account" 
                className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" 
              />
            </div>
            ...
            {/* Invoice ID */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Invoice ID</div>
              <input 
                type="text" 
            ...
            {/* PayPal Reference ID */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">PayPal Reference ID</div>
              <input 
                type="text" 
                placeholder="PayPal Reference ID" 
                className="w-full px-4 py-3 rounded-2xl border border-gray-300 text-sm focus:border-teal-400" 
              />
            </div>
            ...
            {/* Event */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Event</div>
        ...
        <table className="w-full min-w-[1200px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              ...
              <th className={thClass}>Merchant Account</th>
              <th className={thClass}>Transaction ID</th>
              <th className={thClass}>Invoice ID</th>
              <th className={thClass}>PayPal Reference ID</th>
              <th className={thClass}>Event</th>
              <th className={thClass}>Start Date</th>
              <th className={thClass}>Finish Date</th>
              <th className={thClass}>Gross Amount</th>
            ...
            {settlements.length === 0 ? (
              ...
              settlements.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className={tdClass}>{item.report_date}</td>
                  <td className={tdClass}>{item.merchant_account}</td>
                  <td className={tdClass}>{item.transaction_id}</td>
                  <td className={tdClass}>{item.invoice_id}</td>
                  <td className={tdClass}>{item.paypal_reference_id}</td>
                  <td className={tdClass}>{item.event}</td>
                  <td className={tdClass}>{item.start_date}</td>
                  <td className={tdClass}>{item.finish_date}</td>
                  <td className={tdClass}>{item.gross_amount}</td>
                  <td className={tdClass}>{item.fee_amount}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoPayPalSettlementReportList;