...
function MagentoShippingReportList() {
  ...
  const shippingData = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === shippingData.length ? [] : shippingData.map((i: any) => i.id)
    );
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 border border-gray-200 rounded-2xl px-5 py-3 bg-gray-50 focus-within:border-teal-400 focus-within:bg-white transition-all w-full md:w-96">
            ...
            <input
              ...
              placeholder="Search by Carrier, Method..."
          ...
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === shippingData.length && shippingData.length > 0}
                  onChange={toggleAll}
                  className="w-4 h-4 accent-white"
                />
              </th>
              <th className={thClass}>Interval</th>
              <th className={thClass}>Carrier/Method</th>
              <th className={thClass}>Orders</th>
              <th className={thClass}>Total Sales Shipping</th>
              <th className={thClass}>Total Shipping</th>
            </tr>
          </thead>
          <tbody>
            {shippingData.length === 0 ? (
              ...
              shippingData.map((item: any, idx: number) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className={tdClass}>{item.carrier_method}</td>
                  <td className={tdClass}>{item.orders}</td>
                  <td className={tdClass}>{item.total_sales_shipping}</td>
                  <td className={tdClass}>{item.total_shipping}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoShippingReportList;