...
function MagentoNewAccountsReportList() {
  ...
  const newAccounts = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === newAccounts.length ? [] : newAccounts.map((i: any) => i.id)
    );
  };
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3 border border-gray-200 rounded-2xl px-5 py-3 bg-gray-50 focus-within:border-teal-400 focus-within:bg-white transition-all w-full md:w-96">
            ...
            <input
              ...
              placeholder="Search by Interval or Customer..."
        ...
        <table className="w-full min-w-[800px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === newAccounts.length && newAccounts.length > 0}
                  onChange={toggleAll}
          ...
          <tbody>
            {newAccounts.length === 0 ? (
              ...
              newAccounts.map((item: any, idx: number) => (
                <tr
                  key={item.id}
                ...
                >
                  ...
                  <td className={tdClass}>{item.new_accounts}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoNewAccountsReportList;