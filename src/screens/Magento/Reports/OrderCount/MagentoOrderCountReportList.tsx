...
function MagentoOrderCountReportList() {
  ...
  const orderCounts = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === orderCounts.length ? [] : orderCounts.map((i: any) => i.id)
    );
  };
        ...
        <table className="w-full min-w-[900px]">
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === orderCounts.length && orderCounts.length > 0}
                  onChange={toggleAll}
          ...
          <tbody>
            {orderCounts.length === 0 ? (
              ...
              orderCounts.map((item: any, idx: number) => (
                <tr
                  key={item.id}
...
export default MagentoOrderCountReportList;