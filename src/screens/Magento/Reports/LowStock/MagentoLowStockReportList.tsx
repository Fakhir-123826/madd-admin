...
function MagentoLowStockReportList() {
  ...
  const lowStock = dummyData.items || [];
  ...
  const toggleAll = () => {
    setSelected(
      selected.length === lowStock.length ? [] : lowStock.map((i: any) => i.id)
    );
  };
      ...
      {showFilters && (
        <div className="bg-gray-100 border-b px-6 py-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            {/* Source Code */}
            <div>
              <div className="text-xs text-gray-500 mb-1 font-medium">Source Code</div>
          ...
          <thead>
            <tr className="bg-gradient-to-r from-teal-400 to-emerald-500">
              <th className="px-6 py-4 w-10">
                <input
                  type="checkbox"
                  checked={selected.length === lowStock.length && lowStock.length > 0}
                  onChange={toggleAll}
              ...
              <th className={thClass}>Source Code</th>
            </tr>
          </thead>
          <tbody>
            {lowStock.length === 0 ? (
              ...
              lowStock.map((item: any, idx: number) => (
                ...
                >
                  ...
                  <td className={tdClass}>{item.source_code}</td>
                </tr>
              ))
            )}
          </tbody>
...
export default MagentoLowStockReportList;