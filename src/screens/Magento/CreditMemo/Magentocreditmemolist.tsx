...
const mockMemos = [
    { id: 1, memo: "000000001", created: "Feb 24, 2026 1:06:37 AM", order: "000000002", orderDate: "Feb 24, 2026 1:06:34 AM", billTo: "Veronica Costello", status: "Refunded", refunded: "$39.64" },
];
...
const MagentoCreditMemoList = () => {
    ...
    const filtered = mockMemos.filter(m =>
        m.memo.includes(search) ||
        m.billTo.toLowerCase().includes(search.toLowerCase()) ||
        m.order.includes(search)
    );
        ...
        <div className="bg-white rounded-xl shadow-sm overflow-hidden">
            ...
            <div className="px-6 py-5 border-b border-gray-100">
                <div className="mb-4">
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">Manage all credit memos</p>
                </div>
                            ...
                            {showColumns && (
                                <>
                                    ...
                                    <div className="absolute right-0 top-10 z-20 bg-white rounded-xl shadow-lg border border-gray-100 p-3 w-44 space-y-2">
                                        {["Credit Memo", "Created", "Order #", "Order Date", "Bill-to Name", "Status", "Refunded"].map(col => (
                ...
                {showFilters && (
                    <div className="mt-4 pt-4 border-t border-gray-100 space-y-5">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 items-end">
                            {[
                                { label: "Credit Memo #", type: "text" },
                                { label: "Order #", type: "text" },
                                { label: "Bill-to Name", type: "text" },
                            ].map((f) => (
                                <div key={f.label}>
                                    <label className="text-xs font-semibold text-gray-600 mb-2 block">{f.label}</label>
                                    <input type={f.type}
                ...
                <table style={{ width: "100%", minWidth: "800px", borderCollapse: "collapse" }}>
                    <thead>
                        <tr style={{ background: "linear-gradient(to right, #38bdf8, #3b82f6)" }}>
                            ...
                            {[
                                { label: "Credit Memo", sortable: true },
                        ...
                        {filtered.length === 0 ? (
                            ...
                            filtered.map((m, idx) => (
                                <tr key={m.id}
                                    ...
                                    <td className={tdClass}>{m.created}</td>
                                    <td className={tdClass}>{m.order}</td>
                                    <td className={tdClass}>{m.orderDate}</td>
                                    <td className={tdClass}>{m.billTo}</td>
                                    <td className="px-4 py-3">
