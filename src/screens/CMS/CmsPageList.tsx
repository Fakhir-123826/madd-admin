...
import { useGetCmsPagesQuery, useSyncCmsPagesMutation } from "../../app/api/CmsSlices/CmsApi";
...
const CmsPageList = () => {
    ...
    const { data: response, isLoading, refetch } = useGetCmsPagesQuery(
        { vendorUuid: selectedVendorUuid, page, per_page: ITEMS_PER_PAGE },
        { skip: !selectedVendorUuid }
    );
    const [syncPages, { isLoading: isSyncing }] = useSyncCmsPagesMutation();

    const handleSync = async () => {
        if (!selectedVendorUuid) return;
        try {
            await syncPages(selectedVendorUuid).unwrap();
            refetch();
        } catch (error) {
            ...
            <div className="bg-white shadow-sm p-6">
                ...
                <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex gap-4 items-center">
                        ...
                        <AddButton
                            label="Add New CMS Page"
                            type="button"
                            onClick={() => navigate("/AddCmsPage", { state: { vendorUuid: selectedVendorUuid } })}
                        />
                    </div>
                </div>
                            ...
                            {isLoading ? (
                                ...
                                pages.map((cmsPage: any, i: number) => (
                                    <tr key={i} className="bg-white shadow-sm hover:shadow-md transition">
                                        ...
                                        <td className={tdBase}>
                                            {new Date(cmsPage.creation_time || cmsPage.created_at).toLocaleDateString()}
                                        </td>
                                        ...
                                        <td className="relative p-4 rounded-r-xl text-right">
                                            ...
                                            <FaEllipsisV 
                                                onClick={() => navigate(`/EditCmsPage/${cmsPage.uuid || cmsPage.id}`, { state: { vendorUuid: selectedVendorUuid } })} 
