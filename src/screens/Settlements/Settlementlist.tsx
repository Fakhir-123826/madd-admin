...
const SummaryCard = ({ label, amount, count, color }: { label: string; amount: number; count: number; color: string }) => (
...
const ActionMenu = ({ settlement, onApprove, onPay, onDispute, onView }: {
    ...
    return (
        <div className="relative">
            <button onClick={() => setOpen(!open)} className="text-gray-400 hover:text-gray-600 transition p-1">
                <FaEllipsisV />
            </button>
            ...
            {open && (
                <>
                    ...
                    <div className="absolute right-0 top-7 z-20 bg-white rounded-xl shadow-lg border border-gray-100 py-1 w-44 text-sm">
                        <button onClick={() => { onView(); setOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-gray-50 text-gray-600">
                            View Details
                        </button>
                        {settlement.status === "pending" && (
                            <button onClick={() => { onApprove(); setOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-blue-50 text-blue-600">
                                Approve
                            </button>
                        )}
                        {settlement.status === "approved" && (
                            <button onClick={() => { onPay(); setOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-green-50 text-green-600">
                                Mark as Paid
                            </button>
                        )}
                        {(settlement.status === "pending" || settlement.status === "approved") && (
                            <button onClick={() => { onDispute(); setOpen(false); }} className="w-full text-left px-4 py-2 hover:bg-red-50 text-red-500">
                                Dispute
                            </button>
                        )}
                    </div>
...
console.log("SETTLEMENTS:", data?.data?.data);
    const [approve] = useApproveSettlementMutation();
    const [updateStatus] = useUpdateSettlementStatusMutation(); // ← Add this
    ...
    // ✅ ADDED: Handle Dispute
    const handleDispute = async (id: number) => {
        try {
            await updateStatus({ id, status: "disputed" }).unwrap();
            showToast("success", "Settlement marked as disputed.");
            refetch();
        } catch (error) {
            showToast("error", "Failed to update settlement status.");
        }
        ...
        <div className="bg-white shadow-sm p-6 min-h-screen">
            ...
            <div className="flex items-center justify-between mb-6">
                ...
                <div className="flex items-center gap-3">
                    <button onClick={() => refetch()} className="h-9 w-9 rounded-xl border border-gray-200 flex items-center justify-center text-gray-400 hover:text-teal-500 transition">
                        <FaSync className="text-xs" />
                    </button>
                    <button onClick={() => navigate(ROUTES.MAGENTO_SETTLEMENT_GENERATE)} className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-teal-400 to-green-400 text-white text-sm font-medium shadow hover:shadow-md transition">
            ...
            <div className="flex gap-6 border-b border-gray-100 mb-4">
                {STATUS_TABS.map(tab => (
                    <button key={tab.value} onClick={() => { setActiveStatus(tab.value); setPage(1); }} className={`pb-2 text-sm transition-colors border-b-2 -mb-px ${activeStatus === tab.value ? "border-teal-500 text-teal-600 font-medium" : "border-transparent text-gray-400 hover:text-teal-500"}`}>
                        {tab.label}
                    </button>
                ))}
            </div>
            ...
            <form onSubmit={handleSearch} className="flex gap-2 mb-4 max-w-sm">
                <div className="relative flex-1">
                    ...
                    <input value={searchInput} onChange={e => setSearchInput(e.target.value)} placeholder="Search by vendor name…" className="w-full pl-9 pr-4 py-2 rounded-xl border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-teal-300 placeholder:text-gray-300" />
                </div>
                ...
                {search && (
                    <button type="button" onClick={() => { setSearch(""); setSearchInput(""); setPage(1); }} className="px-3 py-2 rounded-xl border border-gray-200 text-sm text-gray-400 hover:text-gray-600">
                        Clear
                    </button>
                )}
            </form>
            ...
            <div className="rounded-t-3xl overflow-x-auto mt-4">
                <table className="w-full text-sm border-separate border-spacing-y-2">
                    ...
                    <tbody>
                        {isLoading ? (
                            <tr><td colSpan={9} className="text-center py-12"><div className="flex items-center justify-center gap-3"><div className="animate-spin rounded-full h-6 w-6 border-b-2 border-teal-500" /><span className="text-gray-400 text-sm">Loading settlements…</span></div></td></tr>
                        ) : isError ? (
                            <tr><td colSpan={9} className="text-center py-12 text-red-400 text-sm"><FaExclamationTriangle className="inline mr-2" />Error loading settlements. Please try again.</td></tr>
                        ) : settlements.length === 0 ? (
                            <tr><td colSpan={9} className="text-center py-16 text-gray-300"><FaMoneyBillWave className="text-4xl mx-auto mb-3 opacity-30" /><p className="text-sm">No settlements found.</p></td></tr>
                        ) : settlements.map(s => (
                            <tr key={s.id} className="bg-white shadow-sm hover:shadow-md transition">
                                <td className={`${tdBase} rounded-l-xl text-xs text-gray-400 font-mono`}>{s.settlement_number}</td>
                                ...
                                <td className={tdBase}>
                                    <div><p className="text-xs text-gray-500">{fmtDate(s.period_start)}</p><p className="text-xs text-gray-400">→ {fmtDate(s.period_end)}</p></div>
                                </td>
                                <td className={`${tdBase} text-gray-700`}>{fmt(s.gross_sales)}</td>
                                <td className={`${tdBase} text-red-400 text-xs`}>
                                    <div><p>Refunds: {fmt(s.total_refunds)}</p><p>Fees: {fmt(s.gateway_fees)}</p><p>Commission: {fmt(s.total_commissions)}</p></div>
                                </td>
                                <td className={`${tdBase} font-bold text-teal-600`}>{fmt(s.net_payout)}</td>
                                <td className={tdBase}>
                                ...
                                <td className={tdBase}>{fmtDate(s.period_end)}</td>
            ...
            {settlements.length > 0 && (
                <div className="flex items-center justify-center gap-2 py-6 text-sm text-gray-600">
                    ...
                    {[...Array(lastPage)].map((_, i) => (
                        <button key={i} onClick={() => setPage(i + 1)} className={`px-3 py-1 rounded-md ${page === i + 1 ? "bg-gradient-to-r from-teal-400 to-green-400 text-white" : "hover:bg-gray-100"}`}>
                            {i + 1}
                        </button>
                    ))}
