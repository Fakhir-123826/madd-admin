...
const Updates = () => {
    ...
    const [toggleMaintenance, { isLoading: toggling }] = useToggleMaintenanceMutation();

    const maintenanceEnabled = maintenanceData?.data?.enabled ?? false;
    ...
    const handleUpdateNow = async () => {
        ...
        try {
            await toggleMaintenance({ enabled: !maintenanceEnabled }).unwrap();
        } catch (err) {
            console.error(err);
        }
                ...
                {isLoading ? (
                    ...
                    <>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {/* Current Version */}
                            <div>
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Current Version</p>
                                <p className="mt-2 text-xl font-semibold text-gray-800">v2.1.4</p>
                            </div>

                            {/* Last Update Date */}
                            <div>
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Last Update Date</p>
                                <p className="mt-2 text-lg font-medium text-gray-700">23 July 2025</p>
                            </div>

                            {/* Status - from real API */}
                            <div>
                                <p className="text-xs font-bold text-gray-500 uppercase tracking-wider">Status</p>
                                <div className="mt-2">
                                    {maintenanceEnabled ? (
                                        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-sm font-medium bg-yellow-100 text-yellow-700">
                        ...
                        <div className="flex justify-end gap-4 mt-8">
                            ...
                            <button
                                ...
                                disabled={toggling}
                                className="flex items-center gap-2 px-6 py-2.5 bg-gradient-to-r from-teal-500 to-green-500 text-white rounded-xl hover:from-teal-600 hover:to-green-600 transition-all duration-200 font-medium shadow-md hover:shadow-lg disabled:opacity-50"
                            >
                                <RefreshCw size={16} />
                                Check for updates
                            </button>
                ...
                <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
                    <div className="bg-white rounded-2xl p-6 w-[350px] text-center shadow-2xl animate-fadeIn">
                        <div className="w-16 h-16 mx-auto mb-4">
                            <div className="animate-spin rounded-full h-16 w-16 border-4 border-gray-200 border-t-teal-500" />
                        </div>
