...
const initialProviders: Provider[] = [
  { id: 1, name: "Shiprocket", type: "Global", mode: "Api", status: "Active" },
  { id: 2, name: "Shiprocket", type: "Global", mode: "Manual", status: "Inactive" },
  { id: 3, name: "Shiprocket", type: "Local", mode: "Manual", status: "Active" },
  { id: 4, name: "Shiprocket", type: "Global", mode: "Api", status: "Active" },
  { id: 5, name: "Shiprocket", type: "Global", mode: "Api", status: "Active" },
  { id: 6, name: "Shiprocket", type: "Local", mode: "Manual", status: "Active" },
  { id: 7, name: "Shiprocket", type: "Local", mode: "Api", status: "Inactive" },
  { id: 8, name: "Shiprocket", type: "Local", mode: "Manual", status: "Active" },
];

const ShippingProviderManagement = () => {
  ...
  const handleAddProvider = () => {
    navigate('/add-shipping-provider');
  };

  
  const handleShipRocket = () => {
    navigate('/add-shiprocket');
  };

  const handleViewProvider = (provider: Provider) => {
    navigate(`/shipping-provider/${provider.id}`, { state: { provider } });
  };
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-lg font-semibold">Shipping Provider Management</h2>
          ...
          <tbody>
            {currentProviders.map((provider, index) => (
              <tr key={index} className="bg-white shadow-sm hover:shadow-md transition relative">
                ...
                {/* Status Cell */}
                <td className="relative p-4 text-gray-600">
                  <span className={`px-3 py-1 rounded-md text-xs font-medium ${statusStyle(provider.status)}`}>
                    {provider.status}
                  </span>
                  <span className="absolute bottom-0 left-0 h-[3px] w-full bg-gradient-to-r from-teal-400 to-green-400" />
                </td>
...
export default ShippingProviderManagement;