...
const initialLanguages: Language[] = [
  { name: "Urdu", iso: "en", rtl: "Yes", default: "Yes", status: "Active" },
  { name: "English", iso: "ur", rtl: "Yes", default: "No", status: "Inactive" },
  { name: "French", iso: "ur", rtl: "Yes", default: "No", status: "Active" },
  { name: "Turkish", iso: "ur", rtl: "Yes", default: "No", status: "Inactive" },
  { name: "Hindi", iso: "ur", rtl: "Yes", default: "No", status: "Active" },
  { name: "Arabic", iso: "ur", rtl: "Yes", default: "No", status: "Inactive" },
];
...
const LanguageManagement = () => {
  ...
  const [deletedLanguageName, setDeletedLanguageName] = useState("");
  ...
  // Filter languages based on filters
  const filteredLanguages = languages.filter(lang => {
    if (filters.status && lang.status !== filters.status) return false;
    if (filters.iso && !lang.iso.toLowerCase().includes(filters.iso.toLowerCase())) return false;
    return true;
  });
  ...
  const handleEditLanguage = (language: Language) => {
    ...
    // navigate('/editlanguage', { state: { language } });
  };
  ...
  const handleConfirmDelete = () => {
    if (selectedLanguage) {
      ...
      setLanguages(languages.filter(l => l.name !== selectedLanguage.name));
      setDeletedLanguageName(selectedLanguage.name);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold">Languages Management</h2>
        </div>
      ...
      <div className="rounded-t-3xl overflow-hidden mt-6">
        <table className="w-full text-sm border-separate border-spacing-y-3">
          ...
          <thead className="bg-gradient-to-r from-teal-400 to-green-400 text-white">
            <tr>
              <th className="p-4 text-left rounded-l-xl">Language Name</th>
              <th className="p-4 text-left">ISO Code</th>
              <th className="p-4 text-left">RTL Support</th>
              <th className="p-4 text-left">Is Default</th>
          ...
          <tbody>
            {currentLanguages.map((lang, index) => (
              <tr key={index} className="bg-white shadow-sm hover:shadow-md transition">
                ...
                <td className={tdBase}>
                  <span
                    className={`px-3 py-1 rounded-md text-xs font-medium ${statusStyle(lang.status)}`}
                  >
                    {lang.status}
                  </span>
                </td>
      ...
      {showDeleteModal && selectedLanguage && (
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
          <div className="bg-white rounded-2xl p-8 w-[450px] text-center relative shadow-2xl animate-[scaleIn_0.3s_ease-out]">
            ...
            <p className="text-gray-600 text-lg mb-2">
              Language <span className="font-semibold text-teal-600">“{deletedLanguageName}”</span>
            </p>
