...
const CurrencyManagement = () => {
  ...
  const [deletedCurrencyCode, setDeletedCurrencyCode] = useState("");
  ...
  // Filter currencies based on filters
  const filteredCurrencies = currencies.filter(currency => {
    if (filters.status && currency.status !== filters.status) return false;
    if (filters.code && !currency.code.toLowerCase().includes(filters.code.toLowerCase())) return false;
    return true;
  });
  ...
  const handleEditCurrency = (currency: Currency) => {
    ...
    // navigate('/editcurrency', { state: { currency } });
  };
  ...
  const handleConfirmDelete = () => {
    if (selectedCurrency) {
      ...
      setCurrencies(currencies.filter(c => c.code !== selectedCurrency.code));
      setDeletedCurrencyCode(selectedCurrency.code);
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h2 className="text-lg font-semibold">Currencies Management</h2>
        </div>
      ...
      <div className="rounded-t-3xl overflow-hidden mt-6">
        <table className="w-full text-sm border-separate border-spacing-y-3">
          ...
          <thead className="bg-gradient-to-r from-teal-400 to-green-400 text-white">
            <tr>
              <th className="p-4 text-left rounded-l-xl">Currency Code</th>
              <th className="p-4 text-left">Symbol</th>
              <th className="p-4 text-left">Conversion Rate</th>
          ...
          <tbody>
            {currentCurrencies.map((currency, index) => (
              <tr key={index} className="bg-white shadow-sm hover:shadow-md transition">
                ...
                <td className={tdBase}>
                  <span
                    className={`px-3 py-1 rounded-md text-xs font-medium ${statusStyle(currency.status)}`}
                  >
                    {currency.status}
                  </span>
                </td>
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/40 backdrop-blur-sm z-50">
          <div className="bg-white rounded-2xl p-8 w-[450px] text-center relative shadow-2xl animate-[scaleIn_0.3s_ease-out]">
            ...
            <p className="text-gray-600 text-lg mb-2">
              Currency <span className="font-semibold text-teal-600">“{deletedCurrencyCode}”</span>
            </p>
